...
class Matrix:
    ...
    def __add__(self, other):
        ...
        for i in range(len(self.matrix)):
            row = []
            for j in range(len(self.matrix[i])):
                row.insert(j, self.matrix[i][j] + other.matrix[i][j])
...
class Coat(Clothes):
    ...
    def size(self, size):
        if isinstance(size, int):
            ...
            print('Принимаем только целые числа!')

    def CalculateConsumption(self):  # для пальто (V/6.5 + 0.5)
        consumption = self.__size / 6.5 + 0.5
        return f'Расход материала на {self.__name}: {consumption:.2f} кв.м.'
...
class Costume(Clothes):
    ...
    def CalculateConsumption(self):  # для костюма (2*H + 0.3)
        consumption = 2 * self.__height / 100 + 0.3
        return f'Расход материала на {self.__name}: {consumption:.2f} кв.м.'

coat1 = Coat(48)
print(coat1.size)
print(coat1.CalculateConsumption())
coat1.size = '54'
coat1.size = 54
print(coat1.CalculateConsumption())
...
print(costume1.CalculateConsumption())
...
class Cell:
    ...
    def __sub__(self, other):
        if self.__cells > other.__cells:
            ...
            return f'Итоговое количество ячеек должно быть положительным'
    ...
    def __str__(self):
        return f'Количество ячеек: {self.__cells}'

    def make_order(self, nums):
        out = ''
        rows, last_row = (self.__cells // nums, self.__cells % nums)
